...
fetches real OHLC from a Python serverless function on Vercel — same spirit as
the Beta_Pf dashboard, adapted to Vercel's current Python runtime:

```
app.py                  ← FastAPI: yfinance pull → JSON snapshot, in-memory + edge cached
index.html              ← loadDataFromAPI() fetches /api/daily_snapshot
requirements.txt        ← fastapi, yfinance, pandas (Vercel auto-installs)
vercel.json             ← cleanUrls + security headers (no build step)
```

> **Why FastAPI and not `api/strategy.py` like Beta_Pf?** Beta_Pf is a *Next.js*
> project, so Vercel's Next preset turns each `api/*.py` into its own function.
> This dashboard is a frameworkless static site, and Vercel's current Python
> builder (CLI 54.x) only auto-detects a single ASGI app named `app` at a
> default entrypoint (`app.py`/`index.py`/…). So the yfinance logic lives in one
> FastAPI app that exposes the JSON at `/api/daily_snapshot` and serves the
> dashboard at `/`. Same outcome: Python + yfinance → JSON → frontend.
...
- `auto_adjust=False` then forward-fill — **matches Alvaro's pipeline.**
  Flip `AUTO_ADJUST = True` in `app.py` for dividend-adjusted closes.
...
`app.py` as the Python entrypoint (FastAPI), installing `requirements.txt` for
you. `app.py` exposes `GET /api/daily_snapshot`; the dashboard at `/` is served
statically by the edge. No `functions` block is needed in `vercel.json`.
...
- [x] Wire live data — Python/yfinance FastAPI app (`app.py`)
